    ...
    async def display_art(self, ctx: commands.Context, card_name: str, variant: Optional[int] = 1):
        try:
            ...
            if exact_match := next((c for c in cards if c.name.lower() == card_name.lower()), None):
                ...
                try:
                    embed, cache_path = self.builder.build_art_embed(exact_match, variant_idx)
                    return await ctx.send(
                        embed=embed,
                        file=discord.File(cache_path, filename=f"{exact_match.id}_{variant_idx}.webp")
                    )
                except FileNotFoundError:
                    return await ctx.send(f"Art file not found in cache for '{exact_match.name}'.")
            ...
            async def handle_final_selection(interaction: discord.Interaction, chosen_card: Pokemon):
                try:
                    ...
                    try:
                        embed, cache_path = self.builder.build_art_embed(chosen_card, variant_idx)
                        try:
                        ...
                        await interaction.response.send_message(
                            embed=embed,
                            file=discord.File(cache_path, filename=f"{chosen_card.id}_{variant_idx}.webp")
                        )
                    except FileNotFoundError:
                        await interaction.response.send_message(
                            f"Art file not found in cache for '{chosen_card.name}'.",
                            ephemeral=True
                        )
                except Exception:
